...
public class ExternalScreen extends GameOptionsScreen {
    ...
    protected void init() {
        ...
        ButtonWidget curseForgeButton = this.addButton(new ButtonWidget(leftSide, height, 150, 20, ModTexts.CURSEFORGE, (buttonWidget) -> {
            Util.getOperatingSystem().open(ModLinks.CURSEFORGE_LINK);
        }));
        curseForgeButton.active = false;

        ButtonWidget modrinthButton = this.addButton(new ButtonWidget(rightSide, height, 150, 20, ModTexts.MODRINTH, (buttonWidget) -> {
            Util.getOperatingSystem().open(ModLinks.MODRINTH_LINK);
        }));
        modrinthButton.active = false;

        height += 24;
        this.addButton(new ButtonWidget(leftSide, height, 150, 20, ModTexts.GITHUB, (buttonWidget) -> {
            Util.getOperatingSystem().open(ModLinks.GITHUB_LINK);
        }));

        this.addButton(new ButtonWidget(rightSide, height, 150, 20, ModTexts.DISCORD, (buttonWidget) -> {
        ...
        height += 24;
        this.addButton(new ButtonWidget(leftSide, height, 150, 20, ModTexts.WEBPAGE, (buttonWidget) -> {
        ...
        this.addButton(new ButtonWidget(rightSide, height, 150, 20, ModTexts.YOUTUBE, (buttonWidget) -> {
            Util.getOperatingSystem().open(ModLinks.DILLON8775_YOUTUBE_CHANNEL_LINK);
        }));

        height += 24;
        ButtonWidget modShowcaseVideo;
        modShowcaseVideo = this.addButton(new ButtonWidget(leftSide, height, 150, 20, ModTexts.MOD_SHOWCASE_VIDEO, (buttonWidget) -> {
