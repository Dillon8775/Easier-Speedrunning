...
public class SafeBootScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addButton(new ButtonWidget(this.width / 2 - 50 - 105, height, 100, 20, ModTexts.FIX_AND_RESTART, (buttonWidget) -> {
            fixOptions();
            info("Fixing options! Re-launch to apply changes.");
            this.client.scheduleStop();
        }));
        this.addButton(new ButtonWidget(this.width / 2 - 50, height, 100, 20, ModTexts.CLOSE_GAME, (buttonWidget) -> {
            info("Closing game! No changes were made.");
            this.client.scheduleStop();
        }));
        this.addButton(new ButtonWidget(this.width / 2 - 50 + 105, height, 100, 20, ModTexts.PROCEED_ANYWAY, (buttonWidget) -> {
